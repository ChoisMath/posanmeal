...
describe("명부 파일 미리보기·확정", () => {
  ...
  it("내보낸 뒤·미리보기 전의 변경은 그 행만 CONFLICT로 다루고 KEEP_SERVER는 서버 값을 지킨다", async () => {
    ...
    const resolved = await resolveImportConflicts(db, fx.main, conflicted.id, [
  ...
  it("지난 학년도는 파일 반영을 거절한다", async () => {
    await db.academicYear.create({ data: { year: 2025, state: "ARCHIVED", version: 0 } });
    const bytes = await renamedStudentFile();
    await expectDomainCode(previewRosterImport(db, fx.main, 2025, "PARTIAL", bytes), "YEAR_MISMATCH");
  });
  ...
  it("취소는 즉시 사본을 비운다", async () => {
    const preview = await previewRosterImport(db, fx.main, YEAR, "PARTIAL", await renamedStudentFile());
    await cancelRosterImport(db, fx.main, preview.id);
