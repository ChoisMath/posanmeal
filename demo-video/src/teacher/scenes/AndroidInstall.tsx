...
export const AndroidInstall: React.FC = () => {
  ...
  const state: AndroidInstallState =
    frame >= launchAt
      ? "app"
      : frame >= homeAt
        ? "home"
  ...
  const rows = [
    [
      "Chrome에서 주소 오른쪽 ⋮ 누르기",
      "meal.posan.kr에 접속한 화면에서 시작해요.",
    ],
    [
      "설치 및 바로가기 만들기 → 설치",
      "메뉴 이름은 Chrome 버전에 따라 다를 수 있어요.",
    ],
    [
      "주소 확인 → 설치 → 완료 기다리기",
      "설치 창에 meal.posan.kr이 맞는지 확인하세요.",
    ],
    [
      "PosanMeal 아이콘으로 바로 접속",
      "홈 화면 또는 앱 목록에서 아이콘을 찾아보세요.",
    ],
  ];
  return (
    <Stage id="AndroidInstall" chapter="안드로이드 · 앱으로 설치하기">
      <Headline size={70}>
        안드로이드라면,
        <br />
        Chrome에서 설치해요.
      ...
      {line === 3 && (
        <Reveal
          delay={lineAt("AndroidInstall", 3, 0.04)}
          style={{
            position: "absolute",
            left: 145,
            top: 850,
