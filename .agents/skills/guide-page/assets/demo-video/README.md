# PosanMeal 안내 영상 · 가이드 이미지 템플릿

`selfstudy/demo-video`의 공용 Remotion·음성·스틸 파이프라인을 복사한 **미설치 템플릿**이다. 프로젝트 루트의 `demo-video/`로 복사한 뒤 그 폴더에서 사용한다. 스킬 폴더에서 직접 설치하거나 렌더하지 않는다. 기존 `demo-video/`가 있으면 필요한 파일을 비교해 합친다.
...
Remotion `4.0.518`과 `mlx-audio==0.5.3`을 원본 잠금 파일로 고정했다. 설치 파일, 모델, 참조 음성, 생성 음성, 다른 프로젝트 가이드는 포함하지 않는다. `SetupCheck`는 환경 점검용 예시로 실제 PosanMeal 화면을 설명하지 않는다. 포함된 길이 JSON은 `--estimate`로 만든 값이며 음성 생성 뒤 실측 값으로 다시 생성해야 한다.
...
새 가이드는 `src/<guide>/`에 원고·타이밍·장면을 만들고 `src/Root.tsx` 및 `scripts/narrate.mjs`의 `GUIDES`에 등록한다. 현재 등록된 가이드는 `setup-check`뿐이다. `appName` 기본값은 `PosanMeal`, `appUrl`은 로컬 확인용 `http://localhost:3000`이다. 실제 제작 시 확인한 주소로 설정한다.
