...
## PosanMeal 안내 영상 적용 범위

이 프로젝트의 기능 안내 영상과 가이드 페이지에는 다음 규칙을 우선 적용한다.
아래 일반 연출 규칙이나 참조 문서와 충돌하면 이 절과
`../guide-page/SKILL.md`의 UI 재사용 규칙을 따른다.

- 실제 앱의 색상, 문구, 메뉴 이름과 업무 흐름을 확인해 목업에 유지한다.
  새 테마로 앱 UI를 재색상화하거나 가상의 기능을 추가하지 않는다.
- 영상과 가이드에서 공유하는 UI 목업에는 grain, vignette, color grade,
  Ken Burns를 적용하지 않는다. 읽고 조작하는 화면은 선명하고 안정적으로
  유지한다. 배경·도입·마무리의 장식 연출은 UI 밖에서만 선택적으로 쓴다.
- 실제 UI의 단색 배경, 정지 상태와 동시 표시가 필요하면 그대로 둔다.
  아래의 five-layer, micro-motion, stagger 규칙을 UI에 강제로 적용하지 않는다.
- 기존 공용 animation과 목업 컴포넌트를 먼저 재사용한다. 새 장면 작업을
  이유로 관련 없는 공용 animation을 임의로 리팩터링하지 않는다.
- 코드 열람은 프로젝트 `AGENTS.md`와 `.codex/PROJECT_MAP.md`에 따라
  관련 화면과 공용 컴포넌트만 읽는다. 앱 전체 `src/`를 먼저 읽지 않는다.
...
## General motion design rules (subject to the PosanMeal scope above)
...
If editing an existing project: read the relevant scenes, shared components and theme
first. Preserve existing shared animation behavior and make only changes needed for
the requested scenes, following the PosanMeal scope above.
