...
| `MonthlyCalendar` | `src/components/MonthlyCalendar.tsx` | 월별 달력, `showType` prop으로 근무/개인 구분. `teacherCalendar`는 교사 확인 탭 전용: 기본 월~금 5열, 주말 버튼으로 7열 전환, 연한 날짜 구분선·토요일 파랑·일요일 빨강. 교사 달력 영역의 터치 스와이프는 왼쪽=다음 달·오른쪽=이전 달이며 짧은 이동·수직 이동·멀티터치·취소는 무시한다. 학생 달력은 기존 7열 유지(스와이프 미적용) |
| `StudentTable` | `src/components/StudentTable.tsx` | 담임 학생관리 표 — 식사별(조/중/석) 컬럼 읽기전용 (미신청=회색 음영/신청=흰색/체크인=식사색 "O"). 학급·월 이동·"N명 선택"/"QR출력"을 상단 한 행에 표시하고 바로 아래 표를 내부 스크롤한다. 제목행·첫 열 체크박스·합계 sticky, 학년도 조회 안내는 표 하단. 셀 구분선은 `stone-400`이며 기존 미신청 음영을 유지한다. 전체선택·행별 선택 `Set<number>` → `StudentQRPrintDialog` 연결 |
