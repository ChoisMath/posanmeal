...
이 문서는 구현·합성 데이터 검증과 실제 운영 적용을 구분한다. 운영 백업, 개인정보 복사, 복원, 서비스 생성, 배포, VERIFIED/READY 변경은 아직 실행하지 않았다. 실제 실행은 [승인된 계획 Task 15](../superpowers/plans/2026-09-19-academic-year-roster.md)의 Step 3~7에 따라 대상과 순서를 검토받은 뒤 진행한다.
...
현재 승인된 것은 계획에 따른 구현·합성 검증이다. 별도 승인 기록에는 다음을 구체적으로 남긴다.
...
로컬 합성 검증·문서 검토를 마친 뒤 이 실행안을 검토받는다. 운영 승인 전에는 push/merge/deploy나 운영 DB·Volume 변경을 실행하지 않는다.
